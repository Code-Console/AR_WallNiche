...
namespace Sample
{
    public class FilesManager : MonoBehaviour
    {
        ...
        //AUGMENTED REALITY MARKET NETWORK FOR HOME, OFFICE DESIGN, AND IMPROVEMENT
        IEnumerator ImageCreate()
        {
            ...
            photo = new Texture2D(Screen.width / 2, Screen.height / 2, TextureFormat.RGB24, false);
            photo.ReadPixels(new Rect(Screen.width / 4, Screen.height / 4, Screen.width / 2, Screen.height / 2), 0, 0, false);
            //photo = new Texture2D((Screen.width *3 ) / 4, (Screen.height * 3) / 4, TextureFormat.RGB24, false);
            //photo.ReadPixels(new Rect(Screen.width / 8, Screen.height / 8, (Screen.width * 3) / 4, (Screen.height * 3) / 4), 0, 0, false);
            photo.Apply();
            Debug.Log("imageCreator => 0~0~" + photo);
